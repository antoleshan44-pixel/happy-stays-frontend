{{-- resources/views/customer/browse-properties.blade.php --}}
@extends('layouts.app')
    ...
    <div class="flex flex-col lg:flex-row gap-6">
                ...
                <form method="GET" action="{{ route('customer.browse') }}">
                    ...
                    <div class="mb-4">
                        ...
                        <select name="property_type" class="filter-input">
                            ...
                            @foreach(['Villa','Apartment','Cottage','House','Cabin'] as $type)
                    ...
                    @if(request()->anyFilled(['location','min_price','max_price','property_type','sort']))
        ...
        <div class="flex-1">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-5">
                ...
                <div class="prop-card"
                     onclick="window.location.href='{{ route('customer.property.detail', $propId) }}'">

                    <div class="prop-card__img-wrap">
                        @if($firstPhoto)
                            <img class="prop-card__img" src="{{ $firstPhoto }}"
                                 alt="{{ $propTitle }}" loading="lazy">
                        @else
                        ...
                        <div class="prop-card__rating">
                            ⭐ {{ number_format($propRating, 1) }}
                        </div>
                        <button class="heart-btn {{ $isSaved ? 'saved' : '' }}"
                                onclick="event.stopPropagation(); toggleWishlist(this, {{ $propId }})">
                        ...
                        <div class="prop-card__footer">
                            <div>
                                <span class="prop-card__price">
                                    KES {{ number_format($propPrice) }}
                                </span>
                                <span class="prop-card__per">/ night</span>
                            </div>
                            <a href="{{ route('customer.property.detail', $propId) }}"
...
@push('scripts')
<script>
const SAVE_URL   = '{{ url("/customer/property") }}';
