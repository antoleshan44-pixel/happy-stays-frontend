    ...
    <div class="grid grid-cols-1 md:grid-cols-12 gap-6 h-[calc(100vh-200px)] min-h-[500px]">
        ...
        <aside class="md:col-span-4 bg-surface-card rounded-xl flex flex-col overflow-hidden shadow-sm border border-border-color">
            ...
            <div class="flex-grow overflow-y-auto custom-scrollbar" id="conversationsList">
                ...
                <div class="p-4 border-b border-border-color hover:bg-gray-50 transition cursor-pointer {{ $loop->first ? 'bg-brand-50 border-l-4 border-l-brand-500' : '' }}"
                    ...
                    <div class="flex gap-3">
                        <div class="relative flex-shrink-0">
                            <div class="w-12 h-12 rounded-full bg-gradient-to-br from-brand-500 to-brand-600 flex items-center justify-center text-white font-semibold">
                                {{ substr($conv['other_user']['name'] ?? 'G', 0, 1) }}
                            </div>
                            <div class="absolute bottom-0 right-0 w-3 h-3 bg-success rounded-full border-2 border-white"></div>
                        </div>
                        <div class="flex-grow min-w-0">
                            <div class="flex justify-between items-start mb-1">
                                <h3 class="text-sm font-semibold text-text-primary truncate">{{ $conv['other_user']['name'] ?? 'User' }}</h3>
                                <span class="text-[10px] text-text-muted">{{ $conv['last_message_time'] ?? '2 min' }}</span>
                            </div>
                            <p class="text-xs text-brand-600 font-medium truncate">{{ $conv['last_message'] ?? 'New message' }}</p>
                            <p class="text-[11px] text-text-muted truncate">{{ $conv['property']['title'] ?? 'Property' }}</p>
                        </div>
                    </div>
                </div>
                @empty
