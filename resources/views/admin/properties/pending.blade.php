...
@section('subtitle', 'Review and approve property listings')
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-0.5">
                {{ count($properties) }} {{ Str::plural('property', count($properties)) }} pending review
            </p>
        </div>
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    @php
                        ...
                        $firstPic   = !empty($property['photos'])
                                    ...
                                    : null;
                    @endphp
                    <tr class="hover:bg-gray-50 transition {{ $riskClass }}" data-id="{{ $property['id'] }}">
                        <td class="px-4 py-4">
                            <input type="checkbox" class="propertyCheckbox rounded border-gray-300
                                   text-blue-600 focus:ring-blue-500" value="{{ $property['id'] }}">
                        </td>
                        <td class="px-4 py-4">
                            <span class="text-sm font-mono text-gray-500">#{{ $property['id'] }}</span>
                        </td>
                        <td class="px-4 py-4">
                            <div class="flex items-center gap-3">
                                <div class="w-12 h-12 rounded-lg overflow-hidden bg-gray-100 flex-shrink-0 border border-gray-200">
                                    @if($firstPic)
                                        <img src="{{ Str::startsWith($firstPic,'http') ? $firstPic : asset('storage/'.$firstPic) }}"
                                             alt="{{ $property['title'] ?? '' }}"
                                ...
                                <div class="min-w-0">
                                    <p class="text-sm font-semibold text-gray-900 truncate max-w-[200px]">
                                        {{ Str::limit($property['title'] ?? 'Untitled', 45) }}
                                    </p>
                                    <p class="text-xs text-gray-500 truncate max-w-[200px] mt-0.5">
                                        {{ $property['location'] ?? 'No location' }}
                                    </p>
                                    <div class="flex items-center gap-2 mt-1">
                                        <span class="text-[10px] text-gray-400">{{ $photoCount }} photo(s)</span>
                                        @if($photoCount < 5)
                        ...
                        <td class="px-4 py-4">
                            <div class="flex items-center gap-2">
                                ...
                                <span class="text-sm text-gray-700">{{ $ownerName }}</span>
                            </div>
                        </td>
                        <td class="px-4 py-4">
                            ...
                            <div class="flex items-center gap-2">
                                <div class="flex-1 bg-gray-200 rounded-full h-1.5 w-16">
                                    <div class="h-1.5 rounded-full transition-all
                                        {{ $riskScore >= 70 ? 'bg-red-500' : ($riskScore >= 50 ? 'bg-yellow-500' : 'bg-green-500') }}"
                                         style="width: {{ min($riskScore, 100) }}%"></div>
                                </div>
                                <span class="text-xs font-medium
                                    {{ $riskScore >= 70 ? 'text-red-600' : ($riskScore >= 50 ? 'text-yellow-600' : 'text-green-600') }}">
                                    {{ $riskScore }}%
                                </span>
                            </div>
                            <p class="text-[10px] text-gray-400 mt-0.5">
                                {{ $riskScore >= 70 ? 'High risk' : ($riskScore >= 50 ? 'Medium' : 'Low risk') }}
                            </p>
                        </td>
                        <td class="px-4 py-4">
                            <p class="text-sm text-gray-600">
                                {{ $createdAt ? \Carbon\Carbon::parse($createdAt)->diffForHumans() : 'Unknown' }}
                            </p>
                            @if($createdAt)
                                <p class="text-[10px] text-gray-400 mt-0.5">
                        ...
                        <td class="px-4 py-4">
                            <div class="flex items-center gap-1.5 flex-wrap">
                                ...
                                <button onclick="showRejectModal({{ $property['id'] }}, '{{ addslashes($property['title'] ?? '') }}')"
                                ...
                                <button onclick="viewDetails({{ $property['id'] }})"
    ...
    if (prop) {
        ...
        if (photos.length > 0) {
            ...
            photos.slice(0, 6).forEach(photo => {
                ...
                galleryHtml += `<img src="{{ asset('storage/') }}/${photoPath}" class="w-full h-20 object-cover rounded-lg" onerror="this.src='https://placehold.co/80x80/f3f4f6/9ca3af?text=?'">`;
            });
            galleryHtml += '</div>';
        }
