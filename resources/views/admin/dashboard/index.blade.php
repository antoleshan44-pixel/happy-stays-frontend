...
@section('subtitle', 'Platform Overview & Key Metrics')
        ...
        <div class="bg-white rounded-xl shadow-sm p-6 hover-lift">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-gray-800 mt-1">{{ number_format($metrics['total_users'] ?? 0) }}</p>
                    <p class="text-green-600 text-xs mt-2">
                        <i class="fas fa-arrow-up"></i> {{ $metrics['user_growth'] ?? 0 }}% increase
                    </p>
                </div>
        ...
        <div class="bg-white rounded-xl shadow-sm p-6 hover-lift">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-gray-800 mt-1">{{ number_format($metrics['total_properties'] ?? 0) }}</p>
                    <p class="text-yellow-600 text-xs mt-2">
                        <i class="fas fa-clock"></i> {{ $pendingCounts['properties'] ?? 0 }} pending
                    </p>
                </div>
        ...
        <div class="bg-white rounded-xl shadow-sm p-6 hover-lift">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-gray-800 mt-1">{{ number_format($metrics['active_bookings'] ?? 0) }}</p>
                    <p class="text-green-600 text-xs mt-2">
                        <i class="fas fa-arrow-up"></i> {{ $metrics['booking_growth'] ?? 0 }}% increase
                    </p>
                </div>
        ...
        <div class="bg-white rounded-xl shadow-sm p-6 hover-lift">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-gray-800 mt-1">{{ CurrencyHelper::formatKES($metrics['monthly_revenue'] ?? 0, true) }}</p>
                    <p class="text-green-600 text-xs mt-2">
                        <i class="fas fa-arrow-up"></i> {{ $metrics['revenue_growth'] ?? 0 }}% growth
                    </p>
                </div>
                ...
                <div class="space-y-4" id="recentActivitiesList">
                    ...
                    <div class="flex items-start space-x-3">
                        <div class="w-8 h-8 bg-green-100 rounded-full flex items-center justify-center flex-shrink-0">
                            <i class="fas fa-{{ $activity['icon'] ?? 'check' }} text-green-600 text-sm"></i>
                        </div>
                        <div>
                            <p class="text-sm text-gray-800">{{ $activity['description'] ?? 'Activity recorded' }}</p>
            ...
            <div class="p-6">
                <div class="space-y-4">
                    <div class="flex items-center justify-between">
                        ...
                        <span class="text-2xl font-bold text-yellow-600">{{ $pendingCounts['properties'] ?? 0 }}</span>
                    </div>
                    <div class="flex items-center justify-between">
                        <div>
                        ...
                        <span class="text-2xl font-bold text-blue-600">{{ $pendingCounts['kyc'] ?? 0 }}</span>
                    </div>
                    <div class="flex items-center justify-between">
                        <div>
                        ...
                        <span class="text-2xl font-bold text-green-600">{{ CurrencyHelper::formatKES($pendingCounts['payouts_amount'] ?? 0, true) }}</span>
                    </div>
                    <div class="flex items-center justify-between">
                        <div>
                        ...
                        <span class="text-2xl font-bold text-red-600">{{ $pendingCounts['disputes'] ?? 0 }}</span>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    var revenueData = {!! $monthlyRevenueData !!};
    var bookingStatusData = {!! $bookingStatusCounts !!};
