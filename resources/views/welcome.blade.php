...
<div class="flex justify-between items-center px-12 py-5 w-full max-w-[1440px] mx-auto">
    <a href="{{ route('home') }}" class="text-2xl font-bold tracking-tighter text-slate-900 font-manrope">Eserian Homes</a>
    <div class="hidden md:flex items-center space-x-8">
        <a href="{{ route('public.browse') }}" class="text-blue-800 font-bold border-b-2 border-blue-800 pb-1 font-manrope font-medium tracking-tight text-sm">Properties</a>
        <a href="{{ route('public.browse') }}" class="text-slate-500 font-medium font-manrope tracking-tight text-sm hover:text-blue-700 transition-all duration-300 ease-out">Collections</a>
        <a href="{{ route('register') }}?role=owner" class="text-slate-500 font-medium font-manrope tracking-tight text-sm hover:text-blue-700 transition-all duration-300 ease-out">List Property</a>
        <a href="{{ route('public.browse') }}" class="text-slate-500 font-medium font-manrope tracking-tight text-sm hover:text-blue-700 transition-all duration-300 ease-out">Concierge</a>
    </div>
    <div class="flex items-center space-x-6">
        <a href="{{ route('login') }}" class="text-slate-500 font-medium text-sm hover:text-blue-700 transition-colors">Sign In</a>
        <a href="{{ route('register') }}" class="text-slate-500 font-medium text-sm hover:text-blue-700 transition-colors">Register</a>
        <a href="{{ route('register') }}?role=owner" class="bg-gradient-to-br from-primary to-primary-container text-white px-6 py-2.5 rounded-md font-manrope font-semibold text-sm scale-95 active:scale-100 transition-transform">
            List Property
        </a>
    </div>
</div>
...
<section class="relative h-[921px] min-h-[700px] flex items-center pt-20">
    ...
    <div class="relative z-10 w-full max-w-[1440px] mx-auto px-12">
        ...
        <!-- Search Form -->
        <form action="{{ route('public.browse') }}" method="GET" class="bg-surface-container-lowest/95 backdrop-blur-xl p-4 rounded-full shadow-2xl flex flex-wrap md:flex-nowrap items-center gap-4 max-w-4xl">
...
<section class="py-24 px-12 max-w-[1440px] mx-auto">
    <div class="flex justify-between items-end mb-12">
        ...
        <a href="{{ route('public.browse') }}" class="text-primary font-bold text-sm flex items-center gap-2 hover:gap-3 transition-all">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
        ...
        <div class="group">
            <a href="{{ route('public.property.detail', $property['id']) }}" class="block">
                <div class="relative overflow-hidden rounded-xl aspect-[4/5] mb-4">
                    @if(isset($property['photos'][0]['photoPath']))
                        <img class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700"
                             src="{{ asset('storage/' . $property['photos'][0]['photoPath']) }}"
                ...
                <div class="flex justify-between items-start">
                    <div>
                        <h3 class="text-lg font-bold font-headline leading-tight">{{ $property['title'] ?? 'Property' }}</h3>
                    ...
                    <div class="text-right">
                        <p class="text-lg font-extrabold font-headline text-primary">KES {{ number_format($property['pricePerNight'] ?? 0) }}</p>
...
<section class="mx-12 my-24 overflow-hidden rounded-3xl relative">
    <div class="bg-gradient-to-br from-primary to-primary-container py-24 px-16 relative z-10">
        <div class="max-w-3xl">
            ...
            <a href="{{ route('register') }}?role=owner" class="inline-block bg-white text-primary px-10 py-4 rounded-md font-bold font-manrope hover:bg-gray-100 transition-all shadow-xl">
                List Your Property
            </a>
        </div>
    </div>
