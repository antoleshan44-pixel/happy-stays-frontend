    ...
    <div class="container-custom py-8">
        <div class="bg-gradient-to-r from-brand-500/10 to-transparent rounded-2xl p-6 md:p-8">
            ...
            <div class="mt-4 flex flex-wrap gap-3">
                <a href="{{ route('owner.dashboard') }}" class="inline-flex items-center gap-2 bg-brand-500 text-white px-4 py-2 rounded-lg text-sm font-semibold hover:bg-brand-600 transition">
                    <span class="material-symbols-outlined text-sm">dashboard</span>
                    Go to Owner Dashboard
                </a>
                <a href="{{ route('owner.property.create') }}" class="inline-flex items-center gap-2 bg-white text-brand-600 border border-brand-200 px-4 py-2 rounded-lg text-sm font-semibold hover:bg-gray-50 transition">
            ...
            <div class="mt-4 flex flex-wrap gap-3">
                <a href="{{ route('admin.dashboard') }}" class="inline-flex items-center gap-2 bg-brand-500 text-white px-4 py-2 rounded-lg text-sm font-semibold hover:bg-brand-600 transition">
                    <span class="material-symbols-outlined text-sm">admin_panel_settings</span>
                    Go to Admin Panel
                </a>
                <a href="{{ route('admin.pending') }}" class="inline-flex items-center gap-2 bg-white text-brand-600 border border-brand-200 px-4 py-2 rounded-lg text-sm font-semibold hover:bg-gray-50 transition">
            ...
            @else
            <div class="mt-4 flex flex-wrap gap-3">
                <a href="{{ route('customer.dashboard') }}" class="inline-flex items-center gap-2 bg-brand-500 text-white px-4 py-2 rounded-lg text-sm font-semibold hover:bg-brand-600 transition">
                    <span class="material-symbols-outlined text-sm">dashboard</span>
                    My Dashboard
                </a>
                <a href="{{ route('customer.bookings') }}" class="inline-flex items-center gap-2 bg-white text-brand-600 border border-brand-200 px-4 py-2 rounded-lg text-sm font-semibold hover:bg-gray-50 transition">
...
<section class="relative min-h-[600px] flex items-center">
    ...
    <div class="relative z-10 container-custom py-16">
        <div class="max-w-3xl">
            ...
            {{-- Search Form --}}
            <form action="{{ route('customer.browse') }}" method="GET" class="bg-white rounded-2xl shadow-xl overflow-hidden">
...
<section class="py-16 bg-surface">
    <div class="container-custom">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-end mb-8">
            ...
            <a href="{{ route('customer.browse') }}" class="mt-4 md:mt-0 text-brand-600 font-semibold text-sm flex items-center gap-2 hover:gap-3 transition group">
                View All Properties
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div class="group bg-surface-card rounded-xl overflow-hidden shadow-sm border border-border-color hover:shadow-lg transition-all hover:-translate-y-1">
                <a href="{{ route('customer.property.detail', $property->id) }}" class="block">
                    <div class="relative overflow-hidden h-64">
                        <img class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105"
                             src="{{ isset($property->photos[0]) ? asset('storage/' . $property->photos[0]->photo_path) : 'https://images.unsplash.com/photo-1582268611958-ebfd161ef9cf?w=800&h=600&fit=crop' }}"
                             alt="{{ $property->title }}">
                        ...
                        <div class="absolute top-3 left-3 bg-black/70 backdrop-blur px-2 py-1 rounded-full flex items-center gap-1">
                            ...
                            <span class="text-xs font-semibold text-white">{{ number_format($property->averageRating ?? 4.5, 1) }}</span>
                        </div>
                    ...
                    <div class="p-4">
                        <h3 class="font-bold text-lg text-text-primary heading-font">{{ Str::limit($property->title, 30) }}</h3>
                        ...
                        <div class="flex justify-between items-center mt-3 pt-2 border-t border-border-color">
                            <div>
                                <span class="text-xl font-bold text-brand-600">KES {{ number_format($property->price_per_night) }}</span>
                                <span class="text-xs text-text-muted">/ night</span>
                            </div>
                            <div class="flex gap-2 text-xs text-text-muted">
                                <span class="flex items-center gap-1">🛏️ {{ $property->bedrooms ?? 2 }}</span>
                                <span class="flex items-center gap-1">🛁 {{ $property->bathrooms ?? 2 }}</span>
                            </div>
                        </div>
                    </div>
                </a>
