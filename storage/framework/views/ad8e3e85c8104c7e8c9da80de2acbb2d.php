...
<?php $__env->startSection('title', is_object($property) ? ($property->title ?? 'Property Details') : ($property['title'] ?? 'Property Details')); ?>
            ...
            <?php if($photoCount > 0): ?>
                <div class="main-image-container">
                    <img id="mainDisplayImage"
                         ...
                         src="<?php echo e($mainPhoto ? (str_starts_with($mainPhoto, 'http') ? $mainPhoto : asset('storage/' . $mainPhoto)) : ''); ?>"
                         alt="<?php echo e($propTitle); ?>"
                ...
                <?php if($photoCount > 1): ?>
                    <div class="thumbnail-gallery">
                        <?php $__currentLoopData = $photos; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $photo): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            ...
                            ?>
                            <?php if($photoPath): ?>
                                <div class="thumbnail-item <?php echo e($index === 0 ? 'active' : ''); ?>"
                                     onclick="changeMainImage('<?php echo e(str_starts_with($photoPath, 'http') ? $photoPath : asset('storage/' . $photoPath)); ?>', this)">
                                    <img class="thumbnail-image"
                                         src="<?php echo e(str_starts_with($photoPath, 'http') ? $photoPath : asset('storage/' . $photoPath)); ?>"
            ...
            <div class="flex-1">
                ...
                <div class="border-b border-border-color pb-5 mb-5">
                    <div class="flex justify-between items-start">
                        <div>
                            <h1 class="text-2xl md:text-3xl font-bold text-text-primary heading-font mb-2"><?php echo e($propTitle); ?></h1>
                            <div class="flex items-center gap-2 text-sm">
                                <span class="material-symbols-outlined text-yellow-500 text-sm">star</span>
                                <span class="font-bold text-text-primary"><?php echo e(number_format($propRating, 1)); ?></span>
                                <span class="text-text-muted">·</span>
                                <span class="text-brand-600 underline cursor-pointer">0 reviews</span>
                                <span class="text-text-muted">·</span>
                                <span class="text-text-secondary"><?php echo e($propLocation); ?></span>
                                <?php if($videoCount > 0): ?>
                                    <span class="text-text-muted">·</span>
                                    <span class="flex items-center gap-1 text-text-muted">
                                        <span class="material-symbols-outlined text-sm">smart_display</span>
                                        <?php echo e($videoCount); ?> video(s)
                                    </span>
                                <?php endif; ?>
                            </div>
                        </div>
                        <div class="flex gap-2">
                            <button onclick="saveProperty(<?php echo e($propId); ?>)" class="p-2 rounded-full hover:bg-gray-100 transition">
                        ...
                        <div class="flex items-center gap-4">
                            <div class="relative">
                                <div class="w-14 h-14 rounded-full bg-brand-500 flex items-center justify-center text-white text-xl font-semibold">
                                    <?php echo e(substr($propOwner, 0, 1)); ?>
                            ...
                            <div>
                                <h3 class="font-bold text-text-primary">Hosted by <?php echo e($propOwner); ?></h3>
                ...
                <!-- Description -->
                <div class="py-5 border-b border-border-color">
                    <p class="text-text-secondary leading-relaxed"><?php echo e($propDesc ?: 'No description provided.'); ?></p>
                </div>
                ...
                <div class="py-5 border-b border-border-color">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <?php $__currentLoopData = array_slice($amenitiesList, 0, 8); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $amenity): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <div class="flex items-center gap-3 text-text-secondary">
                                <span class="material-symbols-outlined text-brand-500 text-sm">check_circle</span>
                                <?php echo e(is_string($amenity) ? $amenity : ($amenity['name'] ?? 'Amenity')); ?>

                            </div>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    </div>
                    <?php if(count($amenitiesList) > 8): ?>
                        <button onclick="showAllAmenities()" class="mt-4 text-brand-600 text-sm font-semibold">
                            Show all <?php echo e(count($amenitiesList)); ?> amenities →
                        </button>
                    <?php endif; ?>
                </div>
                ...
                <div class="py-5">
                    <div class="flex items-center gap-4 mb-5">
                        <div>
                            <div class="text-3xl font-bold text-text-primary"><?php echo e(number_format($propRating, 1)); ?></div>
                            <div class="flex text-yellow-400 mt-1">
                                <?php for($i = 1; $i <= 5; $i++): ?>
                                    <span class="material-symbols-outlined text-sm"><?php echo e($i <= $propRating ? 'star' : 'star_border'); ?></span>
                                <?php endfor; ?>
                            </div>
                            <div class="text-sm text-text-muted mt-1">0 reviews</div>
                        </div>
                    </div>
                    <a href="<?php echo e(route('customer.property.reviews', $propId)); ?>" class="inline-flex items-center gap-2 text-brand-600 font-semibold text-sm">
                        Read all reviews
                ...
                <div class="sticky top-24 p-5 bg-white rounded-xl shadow-lg border border-border-color">
                    <div class="flex justify-between items-baseline mb-5">
                        <div class="flex items-baseline gap-1">
                            <span class="text-2xl font-bold text-brand-600">KES <?php echo e(number_format($propPrice)); ?></span>
                    ...
                    <form method="POST" action="<?php echo e(route('customer.create.booking', $propId)); ?>" id="bookingForm">
                        <?php echo csrf_field(); ?>
                        ...
                        <div id="priceBreakdown" class="space-y-2 text-sm text-text-secondary mb-4">
                            <div class="flex justify-between">
                                <span>KES <?php echo e(number_format($propPrice)); ?> x <span id="nightCount">0</span> nights</span>
...
<script>
    const pricePerNight = <?php echo e($propPrice); ?>;
    const propertyId = <?php echo e($propId); ?>;
    <?php if($featuredVideo): ?>
        const videoUrl = '<?php echo e(str_starts_with($featuredVideo, 'http') ? $featuredVideo : asset('storage/' . $featuredVideo)); ?>';
    <?php else: ?>
        const videoUrl = '';
    <?php endif; ?>
    ...
    function saveProperty(id) {
        fetch('<?php echo e(url("/customer/property")); ?>/' + id + '/save', {
            method: 'POST',
            headers: {
    ...
    function contactHost() {
        window.location.href = '<?php echo e(route("customer.messages")); ?>?host_id=<?php echo e($propId); ?>';
    }

    function showAllAmenities() {
        const amenitiesList = <?php echo json_encode($amenitiesList, 15, 512) ?>;
        let modalHtml = `
    ...
    async function submitBookingForm() {
        ...
        try {
            ...
            if (response.redirected) {
                ...
                else if (data.booking_id || data.id) window.location.href = '<?php echo e(url("/payment/mpesa")); ?>/' + (data.booking_id || data.id);
                else if (data.url) window.location.href = data.url;
                else window.location.href = '<?php echo e(route("customer.bookings")); ?>';
            } else {
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\eserian-homes\resources\views/customer/property-detail.blade.php ENDPATH**/ ?>